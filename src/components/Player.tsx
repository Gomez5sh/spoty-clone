...
const SongControl = ({ audio }: any) => {
  ...
  const formatTime = (time: number) => {
    ...
    const secongs = Math.floor(time % 60);
    const minutes = Math.floor(time / 60);

    return `${minutes}:${secongs.toString().padStart(2, "0")}`;
  };
...
const VolumentControl = () => {
  const setVolume = usePlayerStore((state) => state.setVolume);
  const volume = usePlayerStore((state) => state.volume);

  const previusVolume = useRef<number>(volume);

  const isVloumeSilence = volume < 0.1;
  ...
  return (
    <div className="flex justify-center gap-x-2 text-white">
      ...
      >
        {isVloumeSilence ? <VolumeSilence /> : <Volume />}
      </button>
      <Slider
        defaultValue={[100]}
        max={100}
...
export function Player() {
  ...
  return (
    <div className="flex flex-row justify-between w-full px-1 flex-1">
